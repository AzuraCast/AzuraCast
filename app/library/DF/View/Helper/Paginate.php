    ...
    public function paginate($pager)
    {
        ...
        if ($pages['pageCount'] > 1)
        {
            ...
            if ($pages['first'] != $pages['current'])
                $return_string .= '<li class="prev"><a href="'.$this->view->routeFromHere(array('page' => $pages['first'])).$query_string.'">&laquo; First</a></li>';
            else
                $return_string .= '<li class="prev disabled"><a href="#">&laquo; First</a></li>';
            
            // Previous page link
            if ($pages['previous'])
                $return_string .= '<li><a href="'.$this->view->routeFromHere(array('page' => $pages['previous'])).$query_string.'">&lt; Previous</a></li>';
            else
            ...
            foreach($pages['pagesInRange'] as $page)
            {
                if ($page != $pages['current'])
                    $return_string .= '<li><a href="'.$this->view->routeFromHere(array('page' => $page)).$query_string.'">'.$page.'</a></li>';
                else
            ...
            // Next page link
            if ($pages['next'])
                $return_string .= '<li><a href="'.$this->view->routeFromHere(array('page' => $pages['next'])).$query_string.'">Next &gt;</a></li>';
            else
                $return_string .= '<li class="disabled"><a href="#">Next &gt;</a></li>';
            
            // Last page link
            if ($pages['last'] != $pages['current'])
                $return_string .= '<li class="next"><a href="'.$this->view->routeFromHere(array('page' => $pages['last'])).$query_string.'">Last &raquo;</a></li>';
            else
