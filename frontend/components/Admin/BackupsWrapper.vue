...
const { data: props, isLoading: propsLoading } =
    useQuery<ApiAdminVueBackupProps>({
        queryKey: [QueryKeys.AdminBackups, "props"],
        queryFn: async ({ signal }) => {
            const { data } = await axios.get<ApiAdminVueBackupProps>(
                propsUrl.value,
                { signal },
            );
            return data;
...
export type BackupSettings = Required<
    Pick<
        Settings,
        | "backup_enabled"
        | "backup_time_code"
...
const {
    ...
    queryFn: async ({ signal }) => {
        const { data } = await axios.get<BackupSettings>(settingsUrl.value, {
            signal,
        });
        return data;
    },
