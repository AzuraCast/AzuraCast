...
export type StationMountRecord = Omit<
    Required<StationMount>,
    "id" | "listeners_unique" | "listeners_total"
> & {
...
export type StationMountHttpResponse = StationMountRecord &
    StationMountExtraData;

export const useStationsMountsForm = defineStore("form-stations-mounts", () => {
    const { record, reset: resetRecord } =
        useResettableRef<StationMountExtraData>({
            intro_path: null,
            links: {
                intro: "",
            },
    ...
    const { r$ } = useAppRegle(
        ...
        },
        {
            validationGroups: (fields) => ({
                ...
                advancedTab: [fields.custom_listen_url, fields.frontend_config],
            }),
        },
    );
