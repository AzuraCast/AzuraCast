...
export const useProfilePropsQuery = () => {
    ...
    return useQuery<ApiStationsVueProfileProps>({
        queryKey: queryKeyWithStation([QueryKeys.StationProfile, "props"]),
        queryFn: async ({ signal }) => {
            const { data } = await axios.get<ApiStationsVueProfileProps>(
                apiUrl.value,
                { signal },
            );
            return data;
...
export const useProfileServicesQuery = () => {
    ...
    return useQuery<StationProfileRequired>({
        queryKey: queryKeyWithStation([QueryKeys.StationProfile, "profile"]),
        queryFn: async ({ signal }) => {
            const { data } = await axiosSilent.get(profileApiUrl.value, {
                signal,
            });
            return data;
        },
