...
export const isValidHexColor = createRule({
    validator: (value: Maybe<string>) =>
        !isFilled(value) || /^#?[0-9A-F]{6}$/i.test(value),
    message: () => {
        const { $gettext } = useTranslate();
        return $gettext(
            "This field must be a valid, non-transparent 6-character hex color.",
        );
    },
});
...
export const { useRegle: useAppRegle } = defineRegleConfig({
    rules: () => {
        ...
        return {
            ...
            url: withMessage(url, $gettext("This field must be a valid URL.")),
            isValidPassword,
            isValidHexColor,
        };
    },
